...
class AutoEncoder(nn.Module):
    ...
    def forward_torchscript(
        self, audio: torch.Tensor, f0: torch.Tensor, loudness: torch.Tensor, add_reverb: bool = True
    ):
        batch = self.encoder.forward_torchscript(audio, f0, loudness)
        latent = self.decoder.forward_torchscript(batch)
        ...
        if self.config.use_reverb and add_reverb:
            audio_out["audio_reverb"] = self.reverb(audio_out)
    
        audio_out["a"] = latent["a"]
        audio_out["c"] = latent["c"]
