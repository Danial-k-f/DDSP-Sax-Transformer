...
class SupervisedAudioData(AudioData):
    def __init__(
        ...
        self.num_frame = int(self.waveform_sec / self.frame_resolution)
    ...
    def __getitem__(self, file_idx):
        ...
        # Modified audio loading to handle different torchaudio versions
        try:
            # Try newer torchaudio API first
            audio, sr = torchaudio.load(
                self.paths[file_idx],
                frame_offset=waveform_from,
                num_frames=self.num_wave
            )
        except TypeError:
            # Fallback for older versions
            audio, sr = torchaudio.load(self.paths[file_idx])
            audio = audio[:, waveform_from:waveform_from+self.num_wave]
        
        audio = audio[0]  # Get mono channel
